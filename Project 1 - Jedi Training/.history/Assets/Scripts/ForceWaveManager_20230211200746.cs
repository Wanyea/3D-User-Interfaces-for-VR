...
public class ForceWaveManager : MonoBehaviour
{
    ...
    public void OnShoot()
    {
        ...
        switch (shootMode)
        {
            case ShootMode.Auto:
                ...
                {
                    timeToFire = Time.time + 1f / forceWavePrefab.GetComponent<ForceLightningBehavior>().fireRate;
                    Shoot();
                }
                break;
    ...
    private void Shoot()
    {
        ...
        GameObject forceLightning = Instantiate(forceWavePrefab, new Vector3(hand.position.x, hand.position.y + yLightningOffset, hand.position.z), Quaternion.identity);
