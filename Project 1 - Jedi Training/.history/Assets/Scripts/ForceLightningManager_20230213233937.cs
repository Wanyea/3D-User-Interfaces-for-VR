...
public class ForceLightningManager : MonoBehaviour
{
    ...
    public void OnShoot()
    {
        ...
        switch (shootMode)
        {
            case ShootMode.Auto:
                ...
                {
                    timeToFire = Time.time + 1.2f / forceLightningPrefab.GetComponent<ForceLightningBehavior>().fireRate;
                    Shoot();
                }
                break;
    ...
    private void Shoot()
    {
        // In the End we will going to shoot a forceLightning
        GameObject forceLightning = Instantiate(forceLightningPrefab, new Vector3(hand.position.x, hand.position.y + yLightningOffset, hand.position.z), Quaternion.identity);
