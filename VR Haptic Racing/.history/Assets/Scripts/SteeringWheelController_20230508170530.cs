...
public class SteeringWheelController : MonoBehaviour
{
    ...
    public void ApplyLocalPositionToVisuals(WheelCollider collider)
    {
        ...
        float speedFactor = Mathf.Clamp01(rb.velocity.magnitude / 10.0f);
        float slipFactor = Mathf.Max(wheelHit.sidewaysSlip, wheelHit.forwardSlip);

        WheelFrictionCurve sidewaysFriction = collider.sidewaysFriction;
        sidewaysFriction.stiffness = maxSidewaysFriction - (maxSidewaysFriction - sidewaysFriction.stiffness) * speedFactor;
        sidewaysFriction.stiffness *= (1.0f - slipFactor);
        collider.sidewaysFriction = sidewaysFriction;

        WheelFrictionCurve forwardFriction = collider.forwardFriction;
        forwardFriction.stiffness = maxForwardFriction - (maxForwardFriction - forwardFriction.stiffness) * speedFactor;
        forwardFriction.stiffness *= (1.0f - slipFactor);
    ...
    void FixedUpdate() 
    {
        ...
        Debug.Log("motor: " + motor + ", brakeMotor: " + brakeMotor);
